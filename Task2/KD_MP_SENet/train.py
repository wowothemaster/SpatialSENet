...
def train(rank, a, h, h_teacher):
    ...
    teacher.load_state_dict(torch.load(a.teacher_ckpt)['generator'])
    teacher.eval()
    ...
    for epoch in range(max(0, last_epoch), a.training_epochs):
        ...
        for i, batch in enumerate(train_loader):
            ...
            with torch.no_grad():
                mag_t, pha_t, com_t = teacher(noisy_mag, noisy_pha)
            ...
            B, C_t, T, Fb = teacher.enc_feat.shape
            _, C_s, _, _ = generator.enc_feat.shape

            assert C_t == 2 * C_s, "Channel ratio not 2:1, pooling not safe"

            feat_t_aligned = teacher.enc_feat.view(B, C_s, 2, T, Fb).mean(dim=2)

            loss_kd_feat = F.mse_loss(
                generator.enc_feat,
                feat_t_aligned
            )
...
def main():
    ...
    parser.add_argument('--input_clean_wavs_dir', default='/home/iasp_guest1/tzx/database/direct')
    parser.add_argument('--input_noisy_wavs_dir', default='/home/iasp_guest1/tzx/code/SpatialNet/SPCUP/fullembeddeddeeper/ckpt/newoutput')
    parser.add_argument('--input_training_file', default='/home/iasp_guest1/tzx/database/Moss/train1.txt')
    parser.add_argument('--input_validation_file', default='/home/iasp_guest1/tzx/database/Moss/vad1.txt')
    parser.add_argument('--checkpoint_path', default='/home/iasp_guest1/tyx/MP_SENet_lgy/ckpt05')
    parser.add_argument('--config', default='/home/iasp_guest1/tyx/MP_SENet_lgy/config1.json')
    ...
    parser.add_argument('--config_teacher', default='/home/iasp_guest1/tyx/MP_SENet_lgy/config_teacher.json')
    parser.add_argument(
        '--teacher_ckpt',
        default='/home/iasp_guest1/tyx/MP_SENet_lgy/ckpt01/g_best',
        help='pretrained teacher generator checkpoint'
    )


    a = parser.parse_args()

    with open(a.config_teacher) as f:
        data_teacher = f.read()

    json_config_teacher = json.loads(data_teacher)
    h_teacher = AttrDict(json_config_teacher)
    ...
    build_env(a.config, 'config1.json', a.checkpoint_path)
    ...
    if h.num_gpus > 1:     
        mp.spawn(train, nprocs=h.num_gpus, args=(a, h, h_teacher))
    else:
        train(0, a, h, h_teacher)
