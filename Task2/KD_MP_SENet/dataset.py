...
class Dataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self._cache_ref_count == 0:
            clean_audio, _ = librosa.load(os.path.join(self.clean_wavs_dir, filename + '.wav.wav'), sr=self.sampling_rate)
            noisy_audio, _ = librosa.load(os.path.join(self.noisy_wavs_dir, filename + '.wav_enh.wav'), sr=self.sampling_rate)
            # clean_audio, _ = librosa.load(os.path.join(self.clean_wavs_dir, filename), sr=self.sampling_rate)
            # noisy_audio, _ = librosa.load(os.path.join(self.noisy_wavs_dir, filename), sr=self.sampling_rate)
