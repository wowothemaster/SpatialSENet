...
class PhysicsFeature(nn.Module):
    def __init__(self, num_freqs, mic_dist=0.08, sr=16000):
        """
        mic_dist: Microphone distance (in meters)
        """
        super().__init__()
        ...
        # Pre-calculate angular frequency Omega = 2 * pi * f
        freqs = torch.linspace(0, sr / 2, num_freqs)
        self.register_buffer("omega", 2 * torch.pi * freqs)

        # target and interference angles
        self.register_buffer("target_angle", torch.tensor(90.0)) 
        self.register_buffer("interf_angle", torch.tensor(40.0))

    def get_phase_diff(self, angle_deg):
        """Calculate phase difference for a given angle"""
        angle_rad = torch.deg2rad(angle_deg)
        # TDOA = (d * cos(theta)) / c
        tau = (self.mic_dist * torch.cos(angle_rad)) / self.c
        # Phase Diff = -omega * tau
        return -self.omega * tau 

    def forward(self, x_stft):
        """
        x_stft: [B, F, T, 4] -> Assuming input is dual-microphone complex spectrogram (Real1, Imag1, Real2, Imag2)
        """
        B, F, T, _ = x_stft.shape
        
        # Extract dual-microphone signals
        m1_r, m1_i = x_stft[..., 0], x_stft[..., 1]
        m2_r, m2_i = x_stft[..., 2], x_stft[..., 3]
        
        # Calculate observed phase difference
        cross_r = m2_r * m1_r + m2_i * m1_i
        cross_i = m2_i * m1_r - m2_r * m1_i
        obs_phase = torch.atan2(cross_i, cross_r) # [B, F, T]
        
        # Get theoretical phase difference
        tgt_phase = self.get_phase_diff(self.target_angle).view(1, F, 1)
        int_phase = self.get_phase_diff(self.interf_angle).view(1, F, 1)
        
        # Compute cosine similarity 
        df_tgt = torch.cos(obs_phase - tgt_phase) 
        df_int = torch.cos(obs_phase - int_phase)
        
        # Stack: [B, 2, F, T]
        return torch.stack([df_tgt, df_int], dim=1)
...
class SpatialNetLayer(nn.Module):
    ...
    def __init__(
        ...
        self.df_proj = nn.Conv1d(2, dim_hidden, kernel_size=1)
        # Cross-band block
    ...
    def forward(self, x: Tensor, att_mask: Optional[Tensor] = None, df_feature: Optional[Tensor] = None) -> Tensor:
        r"""
        Args:
            x: shape [B, F, T, H]
        ...
        if df_feature is not None:
            ...
            # 1. Adjust the dimensions for Conv1d: [B*F, 2, T]
            df = df_feature.permute(0, 2, 1, 3).reshape(B*F, 2, T)
            
            # 2. Projection: 2 -> H
            df_emb = self.df_proj(df) # [B*F, H, T]
            
            # 3. Restore the dimensions: [B, F, T, H]
            df_emb = df_emb.transpose(1, 2).reshape(B, F, T, H)
            
            # 4. Residual bias addition
            x = x + df_emb
...
class SpatialNet(nn.Module):
    ...
    def __init__(
        ...
        super().__init__()
        self.physics = PhysicsFeature(num_freqs=num_freqs, mic_dist=mic_dist, sr=sr)
        # Encoder
    ...
    def forward(self, x: Tensor, return_attn_score: bool = False) -> Tensor:
        ...
        for m in self.layers:
            ...
            x, attn = m(x, df_feature=df_feature)
...
if __name__ == '__main__':
    # CUDA_VISIBLE_DEVICES=5, python -m models.arch.SpatialNet
    x = torch.randn((2, 129, 251, 256))  # 251 = 4 second; 129 = 8 kHz; 257 = 16 kHz
