    ...
    def stft(self, x: Tensor) -> Tuple[Tensor, int]:
        ...
        if x.is_cuda:
            with torch.autocast(device_type=x.device.type, dtype=torch.float32):  # use float32 for stft & istft
                X = torch.stft(x, n_fft=self.n_fft, hop_length=self.n_hop, win_length=self.win_len, window=self.window.to(x.device), return_complex=True)
        else:
        ...
        if X.is_cuda:
            with torch.autocast(device_type=X.device.type, dtype=torch.float32):  # use float32 for stft & istft
                ...
                for b in range(X.shape[0]):
                    xb = torch.istft(X[b], n_fft=self.n_fft, hop_length=self.n_hop, win_length=self.win_len, window=self.window.to(X[b].device), length=original_len)
                    xs.append(xb)
