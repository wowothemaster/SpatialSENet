...
class DenseEncoder(nn.Module):
    def __init__(self, h, in_channel):
        ...
        dense_depth = int(getattr(h, "dense_depth", 4))
        self.dense_block = DenseBlock(h, depth=dense_depth)
...
class TSTransformerBlock(nn.Module):
    def __init__(self, h):
        ...
        n_heads = int(getattr(h, "n_heads", 4))

        if h.dense_channel % n_heads != 0:
            raise ValueError(f"h.dense_channel({h.dense_channel}) 必须能被 n_heads({n_heads}) 整除")

        self.time_transformer = TransformerBlock(d_model=h.dense_channel, n_heads=n_heads)
        self.freq_transformer = TransformerBlock(d_model=h.dense_channel, n_heads=n_heads)
...
class MPNet(nn.Module):
    def __init__(self, h, num_tsblocks=None):
        ...
        if num_tsblocks is None:
            num_tsblocks = int(getattr(h, "num_tsconformers", 4))
